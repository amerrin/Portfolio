...
public class RestaurantPanel extends JPanel {
    ...
    private CashierAgent cashier = new CashierAgent("D. Murdock");
    private Vector<MarketAgent> markets = new Vector<MarketAgent>(Arrays.asList(new MarketAgent("m1",cook),new MarketAgent("m2",cook),new MarketAgent("m3",cook)));
	...
	if(type.equals("Customers")){
	    ...
	    if(name.toLowerCase().contains("sd")){
		    SDWaiterAgent w = new SDWaiterAgent(name, aStarTraversal, restaurant, tables);
		    w.setHost(host);
		    w.setCook(cook);
		    w.setCashier(cashier);
		    w.setTurnTable(turnTable);
		    host.setWaiter(w);
		    waiters.add(w);
		    w.startThread();
	    }
